...
export default function SignGroups() {
    ...
    const getCategories = async () => {
        // setLoading(true);
        try {
            const resp = await SignGroupsAPI.getList();
    ...
    const handleDelete = async (id) => {
        setModalOpen(false)
        await SignGroupsAPI.destroy(id);
    ...
    return (
        <ComponentCard newButton={{title: 'Նոր Խումբ', link: '/road-sign-groups/new'}} title="Տեսական թեստերի խմբեր">
            {showSuccess && (
                <SuccessNotification
            ...
            />
            <Table titles={titles} data={categories} goToEdit={(id) => navigator(`/road-sign-groups/${id}`)}
